...
public interface FavoriteMerchantMapper extends BaseMapper<FavoriteMerchant> {
    ...
    @Select("SELECT m.* FROM favorite_merchant f JOIN merchant m ON f.merchant_id = m.merchant_id WHERE f.user_id = #{userId} ORDER BY f.create_time DESC LIMIT #{offset}, #{pageSize}")
    List<Merchant> selectMerchantsByUser(@Param("userId") Long userId,
                                         @Param("offset") int offset,
                                         @Param("pageSize") int pageSize);

    @Select("SELECT COUNT(1) FROM favorite_merchant WHERE user_id = #{userId} AND merchant_id = #{merchantId} LIMIT 1")
