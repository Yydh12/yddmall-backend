...
public interface CommentMapper extends BaseMapper<Comment> {
    ...
    @Select("<script>" +
            "SELECT c.comment_id, c.item_id, i.item_no AS itemNo, c.sku_id, c.user_id, c.rating, c.content, c.image_urls AS imageUrls, COALESCE(c.reply_content, '') AS reply_content, c.reply_time AS reply_time, c.create_time, c.status, " +
            "u.username AS user_name, u.avatar, i.title AS item_title, i.pic_url AS item_pic_url " +
            "FROM item_comments c " +
            "LEFT JOIN \"user\" u ON c.user_id = u.id " +
            "LEFT JOIN item i ON c.item_id = i.item_id " +
